...
InitialMap::InitialMap(Mat &src, int addition_point_interval, int max_corners): PathPlanning(src){
	this->img = src;
...
// get obstacle points
void InitialMap::addtionPoint(){
	vector<vector<Point>> contours;
	int count;

	// find the contours
	findContours(img, contours, noArray(), RETR_LIST, CHAIN_APPROX_NONE);
	// drawContours(img_rgb, contours, 117, Scalar(255, 255, 0));
	...
	for (int i = 0; i< contours.size(); ++i){
		...
		circle(img_rgb, contours[i][0], 2, Scalar(0, 0, 255), -1);	
		for(int j = 0; j< count; ++j){
			points.push_back(contours[i][(j+1)*(addition_point_interval) - 1]);
			circle(img_rgb, contours[i][(j+1)*(addition_point_interval)-1], 2, Scalar(0, 0, 255), -1);
		}
	}
}
