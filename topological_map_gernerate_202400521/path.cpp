...
PathPlanning::PathPlanning(Mat &img){
    this->path_img = img;
    this->cost_map = img;
    cvtColor(path_img, path_img_rgb, COLOR_GRAY2BGR);
}
...
void PathPlanning::delaunay(){
	Size size = path_img.size();
	...
	for (size_t i = 0; i < triangleList.size(); i++){
		...
		if (rect.contains(tmp_point[0]) && rect.contains(tmp_point[1]) && rect.contains(tmp_point[2])){
			...
			line(path_img_rgb, tmp_point[0], tmp_point[1], delaunay_color, 1, LINE_AA, 0);
			line(path_img_rgb, tmp_point[1], tmp_point[2], delaunay_color, 1, LINE_AA, 0);
			line(path_img_rgb, tmp_point[2], tmp_point[0], delaunay_color, 1, LINE_AA, 0);
		}
	}
...
void PathPlanning::voronoi(){
	...
	int channel_num = 0;
	uint8_t* pixelPtr = (uint8_t*)path_img.data;
	...
	for (size_t i = 0; i < facets.size(); ++i){
		...
		for (size_t j = 0; j < facets[i].size(); ++j) {
			...
			if (tmp_point.x < path_img.size().width && tmp_point.y < path_img.size().height && tmp_point.x > 0 && tmp_point.y > 0) {
				// same as path_img.at<uint_8>(ifacet[j].x, ifacet[j].y);
				// but it is faster (usuallay)
				bgrPixel.val[0] = pixelPtr[ ifacet[j].y*path_img.cols*channel_num + ifacet[j].x*channel_num + 0 ];

				// remove the point that is in obstacle
				if (bgrPixel.val[0] == 255){
					voronoi_points.push_back(tmp_point);
					// draw the voronoi points
					// circle(path_img_rgb, Point(tmp_point.x, tmp_point.y), 5, Scalar(255, 0, 0), -1);
				}
			}
		}
		// draw the polygon
		// polylines(path_img_rgb, ifacet, true, Scalar(0, 0, 255), 1, LINE_AA, 0);
	...
	for (int i = 0; i< leadingEdgeList.size(); ++i){
		...
		for (int k = 0; k< 3; ++k){
			...
			if (flag) {
				...
				line(path_img_rgb, vertex_org, vertex_dst, Scalar(255, 0, 255), 1, LINE_4);
			}
	...
	if (test_p == 1) {
		...
		for (int i = 0; i < num_match.size(); i++) {
			full_vex_edge[i].origin = num_match[i];
			full_vex_edge[i].num = i+1;
			full_vex_edge[i].x = subdiv.getVertex(num_match[i]).x; 
			full_vex_edge[i].y = subdiv.getVertex(num_match[i]).y;
		}
		cout << "num_change.size(): " << num_change.size() << endl;
		for (int i = 0; i < num_change.size(); i++) {
			full_vex_edge[i].edge_link_num[0] = num_change[i].x;  
			full_vex_edge[i].edge_link_num[1] = num_change[i].y;  
			full_vex_edge[i].edge_link_origin[0] = match_final[i].x; 
			full_vex_edge[i].edge_link_origin[1] = match_final[i].y; 
			full_vex_edge[i].edge_link_pos[0] = subdiv.getVertex(match_final[i].x).x; 
			full_vex_edge[i].edge_link_pos[1] = subdiv.getVertex(match_final[i].x).y;
			full_vex_edge[i].edge_link_pos[2] = subdiv.getVertex(match_final[i].y).x;
			full_vex_edge[i].edge_link_pos[3] = subdiv.getVertex(match_final[i].y).y;

		}
		full_vex_edge[0].vex_size = num_match.size();
		full_vex_edge[0].edge_size = num_change.size();
		test_p = 2;
	}
	//re-plan section
	else {  
...
void PathPlanning::setPathImg(Mat &inputImg){
    inputImg.copyTo(this->path_img);
}

void PathPlanning::setPathRGBImg(Mat &inputImg){
    inputImg.copyTo(this->path_img_rgb);
}

cv::Mat PathPlanning::getPathRGBMap(){
    return path_img_rgb;
}

void PathPlanning::aStar(){
	Mat a_star_img = path_img;
}
...
bool PathPlanning::isThroughObstacle(Point a, Point b, int thickness){
	...
	// if(min_x < 0 || min_y < 0 || max_x >= path_img.cols || max_y > path_img.rows){
	// 	return true;
	// }

	Mat tmp_img(path_img.size(), CV_8UC1, Scalar(255));
	line(tmp_img, a, b, Scalar(0), thickness);

	int channel_num1 = path_img.channels();
	int channel_num2 = tmp_img.channels();
	uint8_t *pixelPtr1 = (uint8_t*)path_img.data;
	uint8_t *pixelPtr2 = (uint8_t*)tmp_img.data;
	Scalar_<uint8_t> bgrPixel1, bgrPixel2;
		
	for (int i = min_x; i<= max_x; ++i){
		for (int j = min_y; j<= max_y; ++j){
			bgrPixel1.val[0] = pixelPtr1[ j*path_img.cols*channel_num1 + i*channel_num1 + 0 ];
...
void PathPlanning::replan_astar(Voronoi_vertex *vex_edge, Mat &outputImg){
	...
	outputImg.copyTo(ori_astar_show);
	...
	if (is_break == 0) {
		...
		for (int i = 0; i < three_min_dis_p.size(); i++) {
			// put the a* algo loss of the 3 points that closest point of the starting points 
	...
	float step_path = 0;
	int sub_p = 0;
	vector<int> cut_path, cut_path1, cut_path2, cut_path3, tmp_cut;
	...
	for (int i = 0; i < cut_path.size() - 1; i++) {
		line(outputImg, Point2f(vex_edge[cut_path[i] - 1].x, vex_edge[cut_path[i] - 1].y), Point2f(vex_edge[cut_path[i + 1] - 1].x, vex_edge[cut_path[i + 1] - 1].y), Scalar(255, 0, 0), 25, LINE_AA);
	}
	for (int i = 0; i < cut_path.size(); i++) {
		circle(outputImg, Point2f(vex_edge[cut_path[i] - 1].x, vex_edge[cut_path[i] - 1].y), 8, Scalar(200, 0, 255), FILLED);
	}
	circle(outputImg, Point2f(vex_edge[init - 1].x, vex_edge[init - 1].y), 10, Scalar(0, 0, 255), FILLED);
	circle(outputImg, Point2f(vex_edge[path_final_j[0] - 1].x, vex_edge[path_final_j[0] - 1].y), 10, Scalar(0, 255, 0), FILLED);
	...
	if (path_final_j.size() > 1) {
		if (abs(vex_edge[cut_path[0] - 1].x - init_pos.x) <= 1 && abs(vex_edge[cut_path[0] - 1].y - init_pos.y) <= 1) {
			...
			r_next_pos.x = vex_edge[cut_path[1] - 1].x;
			r_next_pos.y = vex_edge[cut_path[1] - 1].y;
			// used for the sake of having the same points
			if (abs(robot_position.x - r_next_pos.x) < 2 && abs(robot_position.y - r_next_pos.y) < 2) {
				robot_position.x = vex_edge[cut_path[1] - 1].x;
				robot_position.y = vex_edge[cut_path[1] - 1].y;
				r_next_pos.x = vex_edge[cut_path[2] - 1].x;
				r_next_pos.y = vex_edge[cut_path[2] - 1].y;
			}
			cout << "r_now_pos1:" << robot_position << " " << "r_next_pos1:" << r_next_pos << endl;

		}
		else {
			robot_position.x = init_pos.x;
			robot_position.y = init_pos.y;
			r_next_pos.x = vex_edge[cut_path[0] - 1].x;
			r_next_pos.y = vex_edge[cut_path[0] - 1].y;
			cout << "r_now_pos2:" << robot_position << " " << "r_next_pos2:" << r_next_pos << endl;
		}
	}
	else {
		robot_position.x = vex_edge[cut_path[0] - 1].x;
	...
	//imshow("go_path_img", go_path_img);
